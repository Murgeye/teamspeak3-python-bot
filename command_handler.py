...
class CommandHandler:
    ...
    def handle_command(self, msg, sender=0):
        ...
        if not msg.startswith('!') and len(msg) > 2:
            teamspeak_bot.send_msg_to_client(self.ts3conn, sender, "Sorry, I only understand defined commands.")
            teamspeak_bot.send_msg_to_client(self.ts3conn, sender, "Use `!help` to get a list of available commands.")
            logger.info("`clid=%s` sent a textmessage without any command: %s", int(sender), str(msg))
            return
        ...
        if handlers is None:
            teamspeak_bot.send_msg_to_client(self.ts3conn, sender, f"Sorry, but I could not find your command `{command}`.")
            teamspeak_bot.send_msg_to_client(self.ts3conn, sender, "Use `!help` to get a list of available commands.")
            logger.info("`clid=%s` sent an unknown command: %s", int(sender), str(msg))

        has_permissions = False
        for handler in handlers:
            if self.check_permission(handler, client_info.ClientInfo(sender, self.ts3conn)):
                has_permissions = True
                handler(sender, msg)

        if not has_permissions:
            teamspeak_bot.send_msg_to_client(self.ts3conn, sender, "You are not allowed to use this command!")
        ...
        if isinstance(event, TextMessageEvent):
            if event.targetmode == "Private":
                if event.invoker_id != int(self.ts3conn.whoami()["client_id"]):  # Don't talk to yourself ...
                    ...
                    self.logger.info("`%s` wrote the following message: %s", str(cl_info.name), str(event.message))

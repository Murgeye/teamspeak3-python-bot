...
class TwitchLive(Thread):
    ...
    def get_servergroup_by_name(self, name="live"):
        ...
        try:
            ...
            TwitchLive.logger.exception("Error while finding a servergroup with the name `%s`.", str(name))
            raise
    ...
    def get_oauth_access_token(self):
        ...
        TwitchLive.logger.info("Got a new Twitch API OAuth Access Token. Expires at %s.", str(self.twitch_api_expires_at))
        ...
        try:
            for client in self.ts3conn.clientlist():
                if int(client.get("client_type")) == 1:
                    TwitchLive.logger.debug("update_client_list ignoring ServerQuery client: %s", str(client))
                    continue
    ...
    def get_clients_with_a_description(self):
        ...
        try:
            for client in self.get_client_list():
                client_description = str(client_info.ClientInfo(client.get('clid'), self.ts3conn).description)

                if not client_description:
                    TwitchLive.logger.debug("The client clid=%s client_nickname=%s has no `client_description`.", int(client.get('clid')), str(client.get('client_nickname')))
                    continue
    ...
    def get_twitch_streamer_user_id(self, client_description):
        ...
        """
        TwitchLive.logger.debug("Getting Twitch streamer user ID from `%s`.", str(client_description))

        twitch_login = client_description.replace("https://www.twitch.tv/", "")
        TwitchLive.logger.debug("Getting Twitch streamer user ID for `login` `%s`.", str(twitch_login))
        ...
        TwitchLive.logger.debug("Got a Twitch streamer user ID: %s.", int(twitch_streamer_user_id))
    ...
    def get_servergroup_ids_by_client(self, cldbid):
        ...
        try:
            ...
            TwitchLive.logger.exception("Failed to get the list of assigned servergroups for the client cldbid=%s.", int(cldbid))
            return client_servergroup_ids

        for servergroup in client_servergroups:
            client_servergroup_ids.append(servergroup.get("sgid"))

        TwitchLive.logger.debug("client_database_id=%s has these servergroups: %s", int(cldbid), str(client_servergroup_ids))
    ...
    def manage_live_status(self, client):
        ...
        """
        TwitchLive.logger.debug("Getting online status of Twitch streamer: `%s`", str(client))
        ...
        if twitch_stream_online:
            ...
            if self.live_servergroup_id not in client_assigned_servergroup_ids:
                if DRY_RUN:
                    TwitchLive.logger.info("Twitch streamer is online. I would have assigned client_database_id=%s the respective servergroup.", int(client['client_database_id']))
                else:
                    TwitchLive.logger.info("Twitch streamer is online. Assigning client_database_id=%s the respective servergroup.", int(client['client_database_id']))

                    try:
                        self.ts3conn._send("servergroupaddclient", [f"sgid={self.live_servergroup_id}", f"cldbid={client['client_database_id']}"])
                    except TS3Exception:
                        TwitchLive.logger.exception("Failed to assign the servergroup to the client_database_id=%s.", int(client['client_database_id']))
                        raise
        else:
            ...
            if self.live_servergroup_id in client_assigned_servergroup_ids:
                if DRY_RUN:
                    TwitchLive.logger.info("Twitch streamer is offline. I would have removed client_database_id=%s from the respective servergroup.", int(client['client_database_id']))
                else:
                    TwitchLive.logger.info("Twitch streamer is offline. Removing client_database_id=%s from the respective servergroup.", int(client['client_database_id']))

                    try:
                        self.ts3conn._send("servergroupdelclient", [f"sgid={self.live_servergroup_id}", f"cldbid={client['client_database_id']}"])
                    except TS3Exception:
                        TwitchLive.logger.exception("Failed to remove the servergroup from the client_database_id=%s.", int(client['client_database_id']))
                        raise
    ...
    def loop_until_stopped(self):
        ...
        while not self.stopped.wait(float(CHECK_FREQUENCY_SECONDS)):
            ...
            try:
                ...
                TwitchLive.logger.error("Uncaught exception: %s", str(sys.exc_info()[0]))
